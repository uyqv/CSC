...
import prog02.GUI;
//how do I switch to linear fib
//ask question about 8
/**
 *
 * @author vjm
 */
public class Main {
  ...
  */
  ...
  public static void doExperiments (Fib fib) {
    ...
    while (true) {
      ...
      if (valid) {
        ...
        ui.sendMessage("Estimated time on input " + n + " is " + estimatedTime + " microseconds.");
        if (estimatedTime > 36000000) {
          ui.sendMessage("The estimated time will take longer than one hour, do you want to " +
                  "continue? \nclick yes or no");
          int answer = ui.getCommand(new String[]{"YES", "NO"});
          switch (answer) {
            case 1:
              break;
          }
      ...
      double accurateTime = accurateTime(fib, n);
      if (valid) {
        percentError = ((accurateTime - estimatedTime) / estimatedTime) * 100;
        ui.sendMessage("fib(" + n + ") = " + fibn + " in " + accurateTime + " microseconds. " +
                percentError + "% error.");
      } else {
        ui.sendMessage("fib(" + n + ") = " + fibn + " in " + accurateTime + " microseconds.");
      }
      fib.recordConstant(n, accurateTime);
      valid = true;
  ...
  public static void doExperiments () {
    ...
    String[] choices = {"ExponentialFib", "LinearFib", "LogFib", "ConstantFib", "MysteryFib",
            "Exit"};
    while (true) {
